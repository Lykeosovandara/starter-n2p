    ...
    <div class="h-screen flex">
        <BackHeader :title="$i18n.t('resetPassword')">
            <n-form ref="formRef" :model="userCredentials" :rules="validationRules">
                ...
                <n-button :disabled="userCredentials.confirmPassword != userCredentials.password" round  @click="addUserHandler" class="w-full">
                    Reset User Password
                </n-button>
            </n-form>
        </BackHeader>
...
const addUserHandler = async (e: MouseEvent) => {
    ...
    if (await formRef.value?.validate()) {
        await userStore.resetPassword(username, userCredentials.value.password);
        router.back();
    }
